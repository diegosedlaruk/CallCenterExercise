...
//		rotLeft(ar, 4);
    ...
    static int countBrives(int[] q) {
    	...
    	//Mientras el array esté desornedado, iteramos
    	while(!arrayInOrder) {
    		
    		int iterator = 0;
    		
			//Asumo que el array no recibirá cambios.
    		...
    		while(iterator < q.length) {
    			...
    			//Si un valor del array no está en orden, implica que si o si se hará 
    			//algún cambio
    			if(!isInTheCorrectPosition(personValue, iterator)) {
    				...
    				//Si el valor siguiente, es menor que el actual, está bien hacer el swap.
    				//Si es mayor, lo dejo donde está. Al avanzar el iterator, evaluaré si a 
    		...
    		//Si no hubo cambios en el array significa que está ordenado.
    		if(!changesMade) {
    			arrayInOrder = true;
    		}
    	}
    ...
    static int minimumSwaps(int[] arr) {
    	...
    	//Movemos los números y los pasamos a la mitad que conviene.
