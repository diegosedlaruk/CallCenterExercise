...
class CallCenterTest {
	...
	void directorAtiendeLlamada() {
			...
			assertTrue(directorMarcos.getCantidadLlamadosAtendidos() == 1);
		}
	

	@Test
	void directorNoAtiendePorSupervisoresYOperadoresLibres() {
			...
			assertTrue(directorMarcos.getCantidadLlamadosAtendidos() == 0);
		}
	
	@Test
	void realizarDiezLlamadas() {
		...
		//Esperamos a que se terminen de procesar las llamadas.
		try {
			Thread.sleep(4 * CALL_DURATION * MILISECONDS);
		} catch (InterruptedException e) {
			
		}
		
		int totalLlamadosAtendidos = operadorLautaro.getCantidadLlamadosAtendidos() + operadorLucas.getCantidadLlamadosAtendidos()
			+ supervisorAlejandro.getCantidadLlamadosAtendidos() + directorMarcos.getCantidadLlamadosAtendidos();
		
		assertTrue(totalLlamadosAtendidos == 10);
